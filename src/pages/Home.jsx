        ...
        <Container className="relative z-10 grid items-center gap-12 py-20 lg:grid-cols-2 lg:py-28">
          <div>
            ...
            <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, delay: 0.1, ease: [0.16, 1, 0.3, 1] }}
              className="mt-2 text-4xl font-extrabold leading-[1.08] tracking-tight sm:text-5xl lg:text-[3.6rem]" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", color: "var(--text)" }}>
            ...
            <motion.p initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, delay: 0.2 }}
              className="mt-6 max-w-lg text-base leading-relaxed" style={{ color: "var(--text-muted)" }}>
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, delay: 0.3 }} className="mt-9 flex flex-wrap gap-4">
            ...
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="mt-7 flex flex-wrap items-center gap-4 text-xs" style={{ color: "var(--text-dim)" }}>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.25 }} className="hidden lg:block">
            <div className="relative">
              ...
              <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
                className="absolute -bottom-6 -right-4 rounded-2xl px-4 py-3" style={{ background: "#ecfdf5", border: "1px solid rgba(5,150,105,0.15)", boxShadow: "0 4px 16px rgba(0,0,0,0.06)" }}>
              ...
              <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut", delay: 1 }}
                className="absolute -top-5 -left-3 rounded-2xl px-4 py-3" style={{ background: "var(--accent-bg)", border: "1px solid rgba(79,70,229,0.12)", boxShadow: "0 4px 16px rgba(0,0,0,0.06)" }}>
                <div className="flex items-center gap-2">
      ...
      <section className="py-20" style={{ background: "var(--surface)" }}>
        <Container className="grid gap-12 lg:grid-cols-[1fr_1.2fr] lg:items-center">
          <motion.div initial={{ opacity: 0, x: -24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            ...
            <h2 className="mt-2 text-3xl font-extrabold leading-snug sm:text-4xl" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", color: "var(--text)" }}>
              A trusted partner for your <span className="gradient-text-accent">end-to-end taxation</span> needs.
            </h2>
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1 }} className="space-y-4">
        ...
        <Container>
          <div className="flex flex-col items-start justify-between gap-5 sm:flex-row sm:items-end">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-extrabold sm:text-4xl" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", color: "var(--text)" }}>Filed right. <span className="gradient-text">On time. Every time.</span></h2>
            </div>
      ...
      <section className="py-24" style={{ background: "var(--surface)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)" }}>
        <Container>
          <div className="max-w-xl">
            ...
            <h2 className="mt-2 text-3xl font-extrabold sm:text-4xl" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", color: "var(--text)" }}>Trust, expertise and <span className="gradient-text">personalized service.</span></h2>
            ...
            {highlights.map((item, i) => {
              ...
              return (
                <motion.div key={item.label} custom={i % 3} variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-50px" }}
                  className="group rounded-2xl p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-card"
      ...
      <section className="relative overflow-hidden py-24">
        ...
        <Container className="relative z-10 flex flex-col items-center gap-6 text-center">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            ...
            <h2 className="mt-3 max-w-2xl text-3xl font-extrabold sm:text-4xl lg:text-5xl" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", color: "var(--text)" }}>
